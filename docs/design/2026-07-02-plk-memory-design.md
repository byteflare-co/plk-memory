...
**西川将弘／Byteflare の実務で同じ判断場面が現実的に再発し、その場面で取得したときに、
どの判断または行動をどう変えるかを具体的に説明できること**とする。他社・別法人・仮想的な
将来利用者にとって再利用可能というだけでは保存しない。
記述的な事実・手順は、判断時に単一の明白な一次情報または通常検索から低コストで同じ答えを
再取得できる場合も保存しない。PLK は一般知識や公式情報のキャッシュではなく、live source の確認で
代替できない非自明な再現知見・高コストな横断整理・検証済み手順だけを `knowhow` 候補とする。
現在構成、ADR、作業履歴、実装上の選択は、それだけではPLKに複製せず既存SoTに残す。過去の
意思決定を保存できるのは、その理由が将来起こりうる具体的な意思決定を変える場合に限る。
そのうえで候補は、(1) 西川将弘／Byteflare で現実的に再発する別セッションでも再利用する規範・判断ルール・検証可能な事実/手順、(2) 一過性でない、
(3) 一次情報・再現実験・人間の明示決定で確定、(4) live lookup で容易に代替できず、既存 SoT の生データ・可変値の複製でない、
...
- **ハードデリート runbook（改訂）**: 手順 1 = **該当シークレットの失効・ローテーション**（Anthropic/Tailscale/freee 各コンソール）。手順 2 以降 = API 停止 → `git filter-repo` → GitHub 側残存（dangling commit・PR diff キャッシュ）は「鍵失効済みのため許容」or Support へ purge 依頼を選択 → 全クライアント re-clone → 全 reindex。利用ログはファクト本文・回答本文・自由記述の判断理由を記録しない。Git backendの検索文previewは既定30日で削除し、PostgreSQL backendは常にhash＋構造化メタデータだけを残す。
...
- **観測テレメトリ**: 検索イベント（返却factのrevision/content hashを含む）と意思決定イベントを分離する。Git backendはprivate JSONL、PostgreSQL backendはtenant RLS付きテーブルへ保存する。検索文hashは恒久保持する。Git backendの平文previewは既定30日で削除し、PostgreSQL backendはDB制約により平文を保存しない。回答本文・自由記述理由は保存しない。ダッシュボードの貢献は因果効果ではなくエージェント自己申告による観測値。
