...
| `plk_assess_candidate` | 自然文候補の保存適格性をread-onlyで判定し、通過時だけ重複候補を検索する | read |
...
なら追加せず、`eligible` の場合だけ重複候補を確認し、新規／更新previewと明示承認を経て
`plk_add` へ進みます。assessment自体は承認でも書き込みでもありません。
